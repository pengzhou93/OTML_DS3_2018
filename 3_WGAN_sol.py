...
pl.scatter(X_train[:,0], X_train[:,1],s = 20, alpha=0.8, edgecolor = 'k', 
           marker = 'o',label='original samples') 
pl.xticks([], [])
pl.yticks([], [])
pl.legend(loc='best')
pl.tight_layout()
...
def generate_images(generator_model,noise_dim, num_samples=1000):
    ...
    pl.figure(figsize=(6,6))
    pl.scatter(X_train[:,0], X_train[:,1],s = 40, alpha=0.2, 
...
# Now we will write the final loop (min discriminator)
# 
...
for epoch in range(500):
    ...
    for i in range(int(X_train.shape[0] // (BATCH_SIZE * TRAINING_RATIO))):
        ...
        for j in range(TRAINING_RATIO):
            # 0.3 is a good value for our toy example
            discriminator_clip(discriminator_model,0.3) 
            ...
            discriminator_model.train_on_batch([sample_batch, noise],
...
# Now we will write the final loop (max discriminator)
# 
